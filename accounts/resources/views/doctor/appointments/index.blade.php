...
@section('title', trans('site.Appointments'))
@push('css')
        ...
        <div class="content-header row">
            <div class="content-header-left col-md-9 col-12 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        <h2 class="content-header-title float-left mb-0">@lang('site.Appointments')</h2>
                    </div>
                </div>
            </div>
        </div>
...
<div class="modal fade" id="emailClient" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
...
<div>
           <label>Message</label>
           <textarea rows="5" name="message" class="w-100 required form-control"></textarea>
</div>
           <button type="button" class="btn btn-primary submit_btn mt-2">
                Submit
            </button>
        </form>
      </div>
...
<script>
    $(document).on('click','.send_mail_client',function(e){
        e.preventDefault();
        let url = $(this).attr('href');
        $('#emailClient').find('form').attr('action',url);
        $('#emailClient').modal('show');
    })
</script>
@endpush
