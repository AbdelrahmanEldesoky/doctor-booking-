    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('action', function ($query) {
                ...
                <a href="'.route('user.appointments.show',$query->id).'" class="btn btn-primary h-100 mr-2"><i class="fa fa-eye"></i></a>
                <form action="'.route('credit').'" method="POST">
                '.csrf_field() .'
                <input type="hidden" value="'.$query->id.'" name="appointment_id">
                <button class="btn btn-success" type="submit" >Send Payment</button>
                </form>
                </div>';
