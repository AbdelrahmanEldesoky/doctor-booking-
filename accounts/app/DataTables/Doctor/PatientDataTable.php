...
class PatientDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('_', function($query){
                return view('doctor.patients.action',get_defined_vars());
            })
            ->rawColumns(['_']);
    ...
    {
        $model = User::query()->with('patients')->where('is_show',1);
        if($this->date)
        {
    ...
    {
        return [
            ['data' => 'name', 'name' => 'name', 'title' => trans('site.name'), 'orderable' => false],
            ['data' => 'email', 'name' => 'email', 'title' => trans('site.email'), 'orderable' => false],
