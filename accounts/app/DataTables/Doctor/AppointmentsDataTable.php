...
class AppointmentsDataTable extends DataTable
{
    ...
    {
        $model = Appointment::query()->with(['doctor','patient','schedule'])->where('doctor_id',Auth::user()->id)->orderBy('from','desc');
        if(session('status'))
        {
        ...
        if(session('date'))
        {
            ...
            $model = $model->whereDate('from',$date);
        }
        if(session('type'))
        {
            $type = session('type');
    ...
    {
        return [
            ['data' => 'patient.name', 'name' => 'patient.name', 'title' => trans('site.patient'), 'orderable' => false,'exportable' => true],
            ['data' => 'patient.email', 'name' => 'patient.email', 'title' => trans('site.email'), 'orderable' => false],
            ['data' => 'schedule.day', 'name' => 'schedule.day', 'title' => trans('site.Day'), 'orderable' => false],
            ['data' => 'from', 'name' => 'from', 'title' => trans('site.Date'), 'orderable' => false],
            ['data' => 'start', 'name' => 'start', 'title' => trans('site.start_time'), 'orderable' => false],
            Column::computed('_')
            ->exportable(false)
            ->printable(false)
            ->searchable(false)
